...
private var backGear : boolean = true;
...
function Update () {
	...
	rigidbody.drag = rigidbody.velocity.magnitude / 120; //
	
	//Debug.Log("Engine RPM"+ inputTorque + " :"+ inputSteer);
	...
	if(EngineRPM > -1){
		...
		NavigateTowardsWaypoint();
			...
			//if the car stop at odd position of Waypoint adjust Automatically.
			FrontLeftWheel.motorTorque = 0;
			FrontRightWheel.motorTorque = 0;
			EngineRPM = 10000;
			inputTorque = inputTorque + 3.0;
...
function NavigateTowardsWaypoint () {
	...
	// that way we can determine how far to the left and right the waypoint is.
	 /*var RelativeWaypointPosition : Vector3  = transform.InverseTransformPoint(Vector3( 
												waypoints[currentWaypoint].ToString.transform.position.x, 
	...
	*/
	 ...
	 var wp: Transform =  waypoints[currentWaypoint];
